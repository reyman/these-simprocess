...
Le front de Pareto pour $\succ \succ$ est plus \textbf{grand} que le front de Pareto pour $ \succ $. Or $\succ \succ$ est \textbf{plus stricte} que $\succ$. Observons de plus près le cas litigieux, c'est à dire pour le point $f$ et $l$, car $f_1 = e_1$
...
On teste tous les éléments de $FP$, le seul qui pose question est $e$ : est ce que $e \succ \succ f$ ?

\textbf{Par conséquent} comme $f_1(e) = f_1(f)$ , $e$ ne domine pas fortement $f$. N'étant pas en mesure d'établir un ordre total entre ces valeurs, il est impossible d'exclure $f$ de $FP$, et donc $ f \in FP_{\succ \succ}$
...
Or (1) est bien $True$ d'après le Tableau 2.3, il est donc possible d'écrire $e_1 \leq f_1$ et $e_2 \leq f_2$


(2) est également $True$ car $e_2 < f_2$
...
On comprend dès lors que chaque minutes qui peut être gagné sur l'exécution de ce modèle est autant de temps gagné sur la rapidité avec laquelle on est capable d'effectuer un retour d'expérience sur notre modèle à partir de ces résultats.


Un triple retour d'experience, pour MGO, pour openMOLE, et pour le modèle.
