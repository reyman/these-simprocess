...
D'autres utilisateurs sont également amenés à travailler au centre, par exemple pour encadrer des développemement, pour des logiciels génériques, ou pour des applications plus spécifiques commandés par les utilisateurs. Genet cite par exemple Jean-Paul Trystram, un directeur de recherche à l'IRIA (ex-INRIA) ayant travaillé au centre avec Xavier Debanne, créateur par la suite du logiciel BDP (Banque de Données Paris 1) utilisé dans ces différentes versions sur le Philips P880.

Pour Genet, le centre de calcul lui a été utile jusque dans les années 1985, date à partir de laquelle les départements d'histoires ont recu un certain nombre de micro-ordinateur sous l'impulsion de Jean-Pierre Bardet, un historien démographe de Paris 4 qui entre ensuite au Ministère de la Recherche . Dans le cas de l'UFR d'histoire de Paris 1 c'est douze Bull 45 qui ont permis aux historiens de relacher l'usage du centre de calcul de Paris 1, et entamer une formation plus systématique des historiens à l'informatique. 

Ce centre fera partie du panel de ressources que les membres de la future équipe P.A.R.I.S n'hésite pas à mobiliser, que cela soit pour des analyses statistiques ou un peu plus tard pour des modèles de simulations (cet usage sera détaillé en particulier dans une prochaine section)
