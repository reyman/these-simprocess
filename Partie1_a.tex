...
Ces algorithmes s'appuient donc sur différents types de stratégies pour tirer partie du peu d'information obtenue via cette fonction $f$. De nature très diverse, on retient pour séparer une première fois ces stratégies une typologie en deux classes. 

\begin{itemize}[label=\textbullet]
\litem{\textit{Probabilistic Approaches}} Les approches stochastiques désignés dans la fig. \ref{fig:S_OverviewOptimisation} sont capables de trouver un optimum assez rapidement, mais ne peuvent pas en garantir la propriété \enquote{globale}
\litem{\textit{Deterministic Approaches}} Les approches déterministes également désignés dans la fig. \ref{fig:S_OverviewOptimisation} peuvent certes garantir au moins théoriquement l'obtention d'un optimum global, mais s'éxecutent souvent au détriment d'un coût computationnel elevé.
\end{itemize}

Ces deux approches partagent également des difficultés communes, et découvre leur limites à des degrés divers en fonction des stratégies mise en oeuvre, dès lors que l'espace de recherche à parcourir devient trop important.
...
Le terme heuristique \textit{heuristic} vient du Grec \textit{heuriskein} que l'on peut traduire par \textit{to find}, ou \textit{to discover}. D'usage plus large que dans la simple discipline informatique, nous retiendrons ici ce terme seulement sous son sens spécifique contextuel à l'optimisation. Rattaché à la définition d'un problème (\textit{problem dependent}), on définit une heuristique comme une mesure approximative pour définir la qualité d'une solution candidate \autocite[34]{Weise2011}.
...
\begin{enumerate}
\item{\textbf{Non reflexive}}  $x_1$ ne peux pas se dominer lui meme
\item{\textbf{assymmétrique}} $ x_1 \succ x_2$ n'implique pas $x_2 \succ x_1$, alors que l'opposé est vrai, $x_1 \succ x_2$ implique $x_2$ ne domine pas $x_1$
\item{\textbf{transitive} }
\end{enumerate}

Différents degrés de dominance ont été développé (\hl{ref fonseca}, comme par exemple la notion de \textit{strong dominance} : $x_1$ domine fortement $x_2$ ($x_1 \succ \succ x_2$) si $x_1$ est strictement meilleur que $x_2$ sur tout ses objectifs. 

Pour bien comprendre comment se construit l'ensemble $X^*$ de solution non dominés $x^* \in \mathbb{X}$ ,on peut étudier en détail comment la dominance se calcule entre les points $e$ et $f$ présentés sur la figure \ref{fig:pranking_a}.
...
\begin{itemize}
\item L'évaluation de la fonction à optimiser est souvent couteuse, comme dans le cas de modèle de simulation dont l'exécution peut prendre jusqu'à plusieurs dizaines de minutes, 
...
\subsubsection{Une petite histoire des Algorithmes EC bio-inspiré}
...
On trouve plus de détails sur l'histoire communes de ces deux voies de recherches du point de vue de l'EC dans les ouvrages de \autocites{DeJong2006a, Fogel1998, Fogel2006a, Fogel2006b, Back1996, Back1997}.
...
\textcite{DeJong2006a} retient trois foyers importants pour le développement de cette deuxième branche dans les années 1960, la \textit{Technical University} de Berlin avec Rechenberg, Biernet et Schwefel \autocite{Beyer2002}, UCLA à la même période avec Lawrence J. Fogel, et l'université du Michigan avec John Holland, eleve de Burks.
...
L'introduction théorique d'une stratégie s'appuyant sur le calcul de l'optimum de Pareto pour définir un classement original des solutions évalués est en effet présenté pour la première fois dans la discipline à la page 197 de \textcite[197]{Goldberg1989}. Une de ces techniques nommé \textit{Non Dominated Sorting} (NDS) \autocite[40-43]{Deb2001}, probablement la plus efficace et plus célèbre, sera reprise et implémenté presque dix ans plus tard en 1994, dans l'algorithme célèbre NSGA (Non dominated Sorting Algorithm) de Deb et Srinivas.

Les travaux de Goldberg ont influencé tout cette génération suivantes de chercheurs, qui se sont par la suite appuyés \autocite[175,235]{Deb2001} sur les informations du calcul de dominance pour développer diverses stratégies d'attribution de \textit{fitness}, comme MOGA (Fonseca et Flemings 1993), NPGA (Horn et Nafpliotis 1994), NSGA (Deb et Srinivas 1994), etc. Ce que l'on peut considérer comme la génération suivantes d'algorithmes, que \textcite{Coello2006} fait démarrer avec l'apparition de l'élitisme \Anote{note_elitisme}, est plus axé encore sur l'efficacité avec notamment l'établissement de standard de mesure et de comparaison \autocites{Coello2006, Zitzler2003}. Parmi ces nouveaux algorithmes, devenu depuis canonique, on trouve PAES (Knwoles and Corne 2000), SPEA (\autocite{Zitzler1999}), ou encore NSGA2 (Deb 2000) etc. On trouve à ce sujet un état de l'art et des exemples de calculs à la main dans un des premiers et très bon ouvrage de synthèse de \textcite{Deb2001}, aux chapitres 5 et 6.

Il est également à noter, comme le fait déjà Golberg en 1989, que cette problématique de recherche d'une solution à un problème multi-critères est d'origine bien plus ancienne, et peut mettre en oeuvre dans son histoire d'autres techniques, plus ou moins similaire à celle de Pareto. Ainsi, les premières traces en EA d'un intérét théorique et parfois pratiques de ces problèmes semblent remonter à Box et Draper (1957), Fogel (1966), Rosenberg (1967) \autocite[174-175]{Deb2001}. Mais la première implémentation informatique est en général attribué à David Schaffer, avec son travail de thèse (1984) et l'implémentation de l'algorithme d'optimisation VEGA (\textit{Vector Evaluated Genetic Algorithm}).

A cela s'ajoute un état de l'art important existant sur l'optimisation multi-objectifs s'appuyant sur Pareto en dehors des techniques évolutionnaire, comme abordé de façon générique dans la section précédente avec les écrits de \autocite{Weise2011}, et dont le livre de synthèse de \textcite[50-79]{Deb2001} donne également un aperçu intéressant, en indiquant également comment certaines de ces stratégies ont aussi été transférées avec plus ou moins de succès aux EA \textcite[171-237]{Deb2001}.
...
\subsubsection{Les principes sous-jacent de ce type d'algorithmes}
...
\subsubsection{Les avantages et inconvénients de ces types d'algorithmes}
...
\hl{-- en travaux a partir de là (24 fevrier) --}

Origine : Deb2001, Fogel2000, Back1997, Dejong2006
...
\begin{enumerate}[label=(\alph*),labelindent=\parindent,leftmargin=*]
	...
	\item Facile à paralléliser (étudié très tot, voire tomassini02 p444)
...
Chacun de ces items ouvre quasiment la voie à des sous domaines d'expertises spécifiques. Voici, un apercu des élements susceptibles de varier d'un algorithme à un autre, et d'une application à une autre, dont certain sont hérités de la nature métaheuristique des EC tel que présenté dans la section précédente \hl{ref section}, et d'autres plus spécifiques aux EC de type multi-objectifs (*) : \autocite[115]{DeJong2006a}[269]{Weise2011}[91]{Liefooghe2010} : 

\begin{itemize}
\item la stratégie de représentation interne d'une solution 
\item la stratégie d'initialisation et de maintien d'une population
\item les stratégies de selection des parents
\item le set d'opérateurs pour la reproduction
\item la fonction de translation \textit{gpm} entre $\mathbb{G}$ et $\mathbb{X}$
...
Si elle résumé bien les principales classes de choix, cette représentation sous forme de liste est encore trop vague, et ne rend pas compte de la plasticité et des contraintes voulu ou imposé par la construction dynamique d'un algorithme adapté au problème. Les dépendances entre items de la liste n'apparaissent pas, alors que chacun des choix réalisés entraine le recalcul des degrés de liberté à disposition de l'expérimentateur, avec l'apparition de nouveaux choix, ou de nouvelles contraintes. Ainsi, le choix d'une représentation du problème sous forme de vecteur de binaire, réel ou encore mixe, de taille dynamique ou fixe, doublé ou non de paramètres spécifiques de convergence, pourra de façon assez logique bouleverser les autres éléments de choix disponibles dans chacunes de ces classes. Il est ainsi très difficile de prévoir l'impact généralisé d'une apparition ou d'une disparition de paramètres sur l'ensemble de ces choix alors même que la combinatoire est elle même encore inconnu, le programme devant rester ouvert et extensible pour l'ajout de nouveaux composants. C'est un problème que nous tenterons de résoudre avec la construction d'une nouvelle librairie logicielle, exposé dans les sections suivantes.

Une partie de la modularité inhérente aux métaheuristique a déjà pu par chance être saisie dans le développement de nombreuses librairies logicielles, il est alors légitime de se poser la question suivante, pourquoi développer et surtout maintenir une nouvelle librairie ? 

Les raisons de ce choix sont guidés par une observation des librairies existantes, et la volonté de répondre au concepts déjà évoqué dans le chapitre 3 :
...
Le couplage de cette librairie avec OpenMOLE doit apparaitre comme totalement transparente à l'utilisateur, tout en restant hautement flexible ce qui suppose l'existence de primitive de plus haut niveau  qui assure la partie parallélisation nécessaire à l'usage confortable de tels algorithmes.
...
\paragraph{Les approches intégrées}

\subparagraph{Le {BehaviorSearch} de Stonedahl}
...
\subparagraph{Les algorithmes EC dans Mason}
...
\paragraph{Les librairies standards}
...
\paragraph{Le choix d'un nouveau couplage, openMOLE et MGO}
...
\subsection{Un nouveau framework MGO (Multi Goal Optimization) pour les EC}
...
\subsubsection{Des principes de conception innovants}
...
\paragraph{La mise en oeuvre du couplage}
