...
Si le terme de HPC semble d'origine récente lorsqu'il est repris par les institutions \autocite{HPCHorizon2020}, on le retrouve déjà employé par les géographes chez \textcite{Turton1998, Openshaw2000, Openshaw2000b} au début des années 2000. En réalité, lorsqu'on s'y intéresse de plus pret en lisant ces ouvrages, on s'apercoit rapidement que cet acronyme n'a vraiment de sens que si on le contextualise. 

\foreignquote {english}{Openshaw2000 The term \enquote{HPC} is very easy: it stands for high-performance computing (or \enquote{computer} depending on context), but the definition of what is \enquote{high-
...
En ce sens, la source brute de calcul impliqué par le terme HPC sous sa forme actuelle correspondra probablement d'ici 20 ans à la puissance d'une micro-ordinateur standard, comme on déjà pu le voir dans le passé sous le coup de la loi de Moore (1965), dont on a fété en 2015 les 50 ans; une loi quasiment devenu pour certains observateurs une prophétie auto-réalisatrice, de part la course à l'innovation qu'elle maintient dans l'industrie et la recherche. 

Qui plus est, créer et utiliser de super-ordinateurs pour réaliser des prouesses en tout sorte impossible autrement (\textit{supercomputers}),avec ou sans paralléllisme des applications, toute ces idées trouvent comme on le verra plus loin des racines évidentes dans les tout début de l'informatique. 

Le HPC ne peut que difficilement être vanté comme provenant d'une quelconque révolution matérielle ou logicielle récente, et ne se substitue qu'à l'expression logique d'un mouvement entamé il y a bien longtemps, et qui s'est largement diversifié par la suite. Se sont ainsi succédés depuis les années 1930 (Z1 mécanique) / 1950 (ENIAC electronique), différentes technologies et surtout différents paradigmes, représentatifs de cette perpetuelle innovation permettant la mise à disposition des utilisateurs des ressources de calculs toujours plus grandes. Le HPC est donc un terme utilisé pour pointer tout autant les anciens (le mythique et monstrueux Cray-1 de 1976) ou les plus récents superordinateurs (ADA ou Turing de l'IDRIS de 2012), mais également depuis les années 1990 la mise en réseau de machine standard à moindre cout accessible à tout le monde (Beowulf cluster, une idée régulièrement renouvellé, aujourd'hui avec les Arduino/Raspberry/Edison); ou encore une puissance répartie sur des milliers d'ordinateurs individuels (le 13 mai 2015 720000 ordinateurs totalise 6,7 PFlops); mais également cette infrastructure supportant à une maille locale, nationale, ou internationale la mise en réseau de tout ce qu'il convient d'apeler aujourd'hui des noeuds de calculs tant leur nature est hétérogène. 

Il y a évidemment des différences dans l'organisation et la gestion de ces différentes ressources, ou les motivations guidant leur bonne utilisation, mais en soit la finalité du projet semble similaire, avec la mise à disposition au plus grand nombre d'une ressource de calcul permettant la résolution la plus rapide possible, et/ou à moindre cout, de problèmes scientifique ou industriel de nature plus ou moins complexe. 

Le terme HPC n'est au final qu'une réminiscence plus moderne d'une course à la performance d'origine bien plus ancienne; un terme parapluie qui accueille sous som ombrelle une diversité de matériels, et de logiciels toujours plus grande. 

L'\textit{European Technology Platform for HPC} (ETP4HPC) créé en 2011, reconnu comme la plateforme de référence pour l'établissement de partenariat public privé et la réalisation des objectifs du projet HPC de l'Horizon2020, définit le HPC ainsi : 
...
- Capability computing refers to the use of a large and high-performing computing infrastructure to solve a sin-gle, highly complex problem in the shortest possible time. Systems used for this purpose are called supercomputers and consist of many tightly coupled compute nodes with distributed memory, all controlled by a single unit.

- Capacity computing refers to optimising the efficiency of using a compute system to solve as many mid-sized or smaller problems as possible at the same time at the low-est possible cost.} \autocite{ETP4HPC2013}

C'est dans la reconnaissance progressive de ces ressources comme levier aujourd'hui indispensable pour définir l'innovation que naissent les derniers projets pour le HPC à l'échelle européenne. Comme en témoigne les intitulés des projets européens, il s'agit aujourd'hui de se positionner dans une course qui n'est pas sans rapeller celle que l'on a connu pour l'espace, où la mutualisation est devenu nécessaire pour \enquote{tenir la distance} face à certaines puissances (la chine possède le numéro 1 du top 500 depuis 2013, avec Tianhe-2 à 33,86 PFlops). C'est du moins ce que transmette les intitulé des projets pour \autocite{HPCHorizon2020} \foreignquote{english}{High-Performance Computing: Europe's place in a global race} que l'on retrouve repris sous une autre forme encore dans le programme de l'ETP4HC \autocites{ETP4HPC2012, ETP4HPC2013} \foreignquote{english}Today, {\enquote{to Out-Compute is to Out-Compete} best describes the role of HPC.}

Une autre façon d'imaginer de tels changements dans notre quotidien est de prendre le problème à l'envers, comme le propose \textcite{Openshaw2000}  \foreignquote{english}{ [...] how would do you research if that PC on your desk was suddenly 10000 times faster and more powerful. It is likely that some researchers would not know what to do with it, some would not want it, but some would spot major new possibilities for using the computer power to do geography (and geo-related science) differently.} Mais comme Openshaw précise ensuite, quel intéret y a t'il de passer d'une opération dont la durée est de 30 minutes à une opération de 10 secondes ? Il nous faut donc un challenge scientifique, une motivation, une façon de montrer que cet effort pour engager la parallélisation des applications est justifié.

Cette motivation, il me semble que les géographes l'ont déjà trouvé une première fois en françe dans les années 1970, avec la découverte des possibilités d'applications permises par l'informatique. Touchant de façon générale les sciences humaines, l’usage de \textit{super computer} est en effet le premier contact que les pionniers ont eu avec l'informatique. A ce titre, le chapitre 1 a donné un aperçu des nombreux travaux réalisés par ces chercheurs en sciences humaines, sans forcément souligner à quel point il a fallu du courage pour braver à plusieurs, ou parfois seul les contraintes, à la fois intellectuelles, financière et physique permettant approcher de telle machines, encore largement gardé par les sciences physiques et mathématique. C'est dans ce contexte que l'on essaiera d'insérer dans la suite de notre esposé un début de réflexion sur les différents appui dont on bénéficié ces pionniers, au travers par exemple de structure comme les centres de calculs.
...
Dans son manifeste pour le HPC \autocite[2]{Openshaw2000} paru la même année que son ouvrage sur la GéoComputation \autocite{Openshaw2000b}, Openshaw et Turton n’hésitent pas à engager les géographes dans une métaphore qui les met en proie à une forme de maladie des plus virulentes. S'en prenant spécifiquement aux géographes \enquote{SIG-istes} (et aussi accessoirement les sciences sociales en général), c’est une forme particulièrement menaçante de \foreignquote{english}{let others do the programming for us} qui condamnerait peu à peu ces scientifiques à delaisser la programation plutôt qu’à s’en emparer pour devenir acteur de leur propre outils \Anote{openshaw_virus}. \textit{Vrai ?} ou \textit{Faux ?}, cette question que l'on imagine volontairement provocatrice, est en réalité loin d'être aussi simple qu'elle n'y parait, l'outil informatique étant définitivement à double tranchant; un argument que n'hésiterons pas à mobiliser les plus fervent critiques envers cette nouvelle science géographique quantitativiste. 

On ne saurait également ignorer les efforts toujours investi par les géographes dans une forme de programmation orienté vers les statistiques (SAS, R) ou la cartographie (R, Flash, et Webmapping en général), ainsi que le combat toujours en cours de certains pour engager les modélisateur dans \enquote{des pratiques de modélisation et de simulation libérées} \autocite{Banos2013}. Les plateformes de modélisation agents plus \textit{user-friendly} apparu dans les années 1990 ayant permis une approche à la fois plus ludique, et plus pragmatique de la programmation : Netlogo, Gama, Repast, etc. 

Sur un tout autre plan, il faut dire que l’arrivée, considérée en général comme un événement bénéfique, des \textit{micro-computer}, à aussi rendu en partie obsolète cette pratique de l’informatique organisée autour de centre de calculs. Quant à l'autonomie et à l'indépendance permises par l'utilisation de cette nouvelle ressource, elles peuvent être également prisent comme une invitation au repli sur soi, à la stérilité, même lorsqu'il s'agit de \enquote{création} ou plutôt de \enquote{re-création} ! Après ne dit on pas régulièrement chez les informaticiens qu'il est inutile de sans cesse vouloir recréer la roue ? 

Avec l’augmentation graduelle et rapide de la puissance disponible sur les micro-ordinateurs dans les années 1980-1990, grand nombre de programmes tournant dix ans plus tôt sur des \textit{mainframes} ou des \textit{superordinateurs} sont remplacés par des logiciels tout à fait fonctionnels sur un PC de puissance modeste (et cela parfois au prix évident d'une selection darwinienne dans la profusion de logiciels crées courant des années 1980, combien de réussite peut on compter par rapport aux efforts engendrés, un peu partout, parfois surement en parallèle ?). 

Si il n'est pas possible de statuer sur les sciences humaines en général, la voie nouvelle offerte par la simulation chez les géographe francais ne fait que révèler aux yeux des premiers modélisateurs ces challenges qu'il faudra résoudre en tenant compte des progrès de l'informatique.

La question posé ici est simple, elle consiste à se demander pourquoi les géographes, et surtout les modélisateurs, devraient il à nouveau s’engager dans cette voie \enquote{difficile} que semble être celle du HPC ? Quel sont ces anciens ou nouveaux challenges que l'on peut imaginer trouver dans l'usage du HPC en simulation ? Et si ils doivent y accéder, par quels moyens peuvent il espérer y arriver aujourd’hui ?
