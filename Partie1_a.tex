...
\paragraph{un modèle théorique : SimpopLocal}
...
Le paramètre de population est réglé par défaut à 125 fourmis dans la colonie, mais il est également possible de le faire varier sur N. 


Si à l'initialisation 

  sera susceptible auquel vous même vous n'aviez pas pensé lors de la création du modèle.

 la définition des valeurs possible de paramètres va exposer votre 

Les fonctions objectifs se base chacune sur le temps écoulé entre le début de la simulation $t_0$ et le temps indiqué pour la disparition complète d'un tas de nourriture $t_n$.
...
Imaginons par exemple que je fasse tourner une première simulation avec des paramètres pris au hasard (population = 60, diffusion = 25, xx) , voici comment la projection se présente entre l'espace des paramètres, et l'espace des objectifs.

Attention toutefois, il faut savoir que l'optimiseur, dans ses multiples tentative pour trouver le meilleur jeu de valeur de paramètres qui mimise les trois fonctions objectifs risque de produire des combinaisons de valeur de paramètres qui poseront problèmes. Effectivement que se passe-t-il dans notre modèle si les paramètres sont les suivants ?

\begin{enumerate}
\item $population = 0, Evaporation-rate = 25.5 , Diffusion-rate=24.6$
\item $population = 250, Evaporation-rate = 0.0 , Diffusion-rate=0.0$
\end{enumerate}

Plusieurs pistes peuvent être indiqué pour éviter ou limiter l'apparition de comportement pouvant bloquer lors de l'exploration :
- Soit vous fixer certains de ces paramètres lorsque c'est possible
- Soit vous identifiez à l'avance les combinaisons de jeu de valeur de paramètres et vous les interdisez à l'optimiseur
- Soit vous introduisez un artefact dans votre modèle limitant le nombre de tortue disponible pour une espèce donnée
- Soit vous introduisez des critères d'arret dans votre modèle beaucoup plus restrictif

Un autre type d'erreur peut encore intervenir, la combinatoire de paramètre pouvant faire apparaitre des erreurs dans le déroulement de votre programme.
