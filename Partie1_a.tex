...
Si le terme de HPC semble d'origine récente lorsqu'il est repris par les institutions, on le retrouve déjà employé par les géographes chez \textcite{Turton1998, Openshaw2000} en 1998. En réalité, le terme en lui même n'a vraiment de sens que si on le contextualise. L'idée de réalisés des calculs en parallèles, ou l'idée même de super-ordinateurs remonte au débuts de la discipline informatique. En ce sens, la ressource brute de calcul se cachant sous le terme HPC aujourd'hui correspondra d'ici 20 ans à la puissance d'une micro-ordinateur standard, comme on déjà pu le voir dans le passé sous le coup de la loi de Moore (1965) qui vient en 2015 de feter ces 50 ans; une loi quasiment devenu prophétie auto-réalisatrice de par la course à l'innovation qu'elle maintient. En soit, le HPC peut difficilement être vanté comme une révolution matérielle récente, car elle n'est que la suite logique d'un mouvement entamé il y a bien longtemps. Se sont ainsi succédés depuis les années 1930 (Z1 mécanique) / 1950 (ENIAC electronique), différentes technologies et surtout différents paradigmes, représentatifs de cette perpetuelle innovation permettant la mise à disposition des utilisateurs des ressources de calculs toujours plus grandes. Le HPC est donc un terme utilisé tout autant pour pointer tout autant les supercomputers vieux (Cray-1 de 1976) ou plus récents (ADA ou Turing de l'IDRIS de 2012), mais également depuis les années 1990 (Beowulf cluster en 1990, une idée régulièrement renouvellé, aujourd'hui avec les Arduino/Raspberry/Edison) la mise en réseau de machine standard à moindre cout accessible à tout le monde; une puissance répartie sur des milliers d'ordinateurs individuels (le 13 mai 2015 720000 ordinateurs totalise 6,7 PFlops); mais également cette structure locale, nationale, internationale supportant la mise en réseau de tout ce qu'il convient d'apeler aujourd'hui des noeuds de calculs tant leur nature est hétérogène. 

L'idée d'une execution en parallèle d'une même instruction sur plusieurs jeu de données (SIMD) est également une idée assez veille, le plus ancien \textit{supercomputer} ayant abordé ce sujet étant le projet Illiac IV demarré en 1965. D'autres paradigmes mène à des architectures différentes, comme l'entreprise visant l'execution de multiples instructions sur de multiples jeux de données (MIMD), étudié dans une période similaire, ce paradigme ci va déboucher sur des prototypes aux architectures engageantes, comme par exemple le Caltech Cosmic Cube 1983 (1\% prix, 10\% performance rapport à un Cray 1) qui malgré un petit nombre de 64 processeurs présente un rapport qualité prix et une architecture précurseur du futur standard MPI qui marque le début d'une fin de reigne des architectures couteuse de type SIMD pour la diffusion d'architecture MIMD, dont on va voir qu'elle sont par la suite totalement démocratisé par des approches comme Beowulf.

 => MPI standard à expliquer

Il y a évidemment des différences dans l'organisation et la gestion de ces différentes ressources, mais l'objectifs reste le même, mettre à disposition une ressource de calcul pour la résolution la plus rapide possible d'un objectif plus ou moins complexe. C'est dans la reconnaissance progressive de ces ressources comme levier aujourd'hui indispensable pour définir l'innovation que naissent les derniers projets pour le HPC à l'échelle européenne. Comme en témoigne les intitulés des projets européens, il s'agit aujourd'hui de se positionner dans une course similaire à celle que l'on connait pour l'espace, où la mutualisation est devenu nécessaire pour tenir la distance face à certaines puissances (la chine possède le numéro 1 du top 500 depuis 2013, avec Tianhe-2 et 33,86 PFlops) : \foreignquote{english}{High-Performance Computing: Europe's place in a global race}
...
Il faut dire que l’arrivée, considérée en général comme un événement bénéfique, des \textit{micro-computer}, à aussi rendu en partie obsolète cette pratique de l’informatique organisée autour de centre de calculs. L’usage de \textit{super computer} n’est en effet pas nouvelle en science humaines et en géographie. A ce titre, le chapitre 1 a donné un aperçu des travaux que de nombreux pionniers en sciences humaines ont effectué, ces derniers étant bien obligé pour cette mise en oeuvre d’approcher de telle machines, en bravant seul ou à plusieurs les contraintes, à la fois intellectuelles, financière et physique imposé par un tel exercice.

Avec l’augmentation graduelle et rapide de la puissance disponible sur les micro-ordinateurs dans les années 1980, grand nombre de programmes tournant dix ans plus tôt sur des \textit{mainframes} sont remplacés par des logiciels tout à fait fonctionnels sur un PC de puissance modeste. 

La question posé ici est simple, elle consiste à se demander pourquoi les géographes devraient il à nouveau s’engager dans cette voie \enquote{difficile} que semble être celle du HPC ? Et si ils doivent y accéder, par quels moyens peuvent il espérer y arriver aujourd’hui ?
...
T : Toutes les conditions sont réunies pour expérimenter ce retour au HPC dans le cadre des modélisations à Geographie Cités

\subsubsection{Tournant des années 1990 dans le HPC}

Depuis le début des années 1980 et surtout courant des années 1990, ce domaine connait pourtant un certain nombre de révolutions, avec par exemple la démultiplication du nombre de processeurs, et/ou l’émergence de nouvelles architectures permettant de penser autrement le parallèlisme. 

On pourrait citer par exemple les travaux de David E. Shaw sur la machine “Non-Von” (pour “Non Von Neumann”) de la Colombia University, ou encore ceux de Danny Hillis au MIT sur la “Connection Machine (CM), tous deux reconnu au début des années 1980 comme étant des pionniers dans la construction d’architecture massivement parallèle. Celui ci travaille avec Minsky et Papert au laboratoire MIT sur le langage LOGO, avant de cré”er une société en 1983 “Thinking Machine” qui distribuera une série de CM (Connection Machine) d’architecture SIMD\Anote{simd_def} puis MIMD\Anote{mimd_def}, point de départ dans la construction de machines composé de plusieurs milliers de processeurs très simple (jusqu'à 65536 processeur de 1 bit / 4kbit mémoire pour la CM-1), il repense la façon dont les processeurs sont ammenés à communiquer entre eux en se focalisant non plus sur la puissance des processeurs mais la flexibilité et l'efficience des communications entre ceux-ci. Sachant David Hillis largement inspiré des travaux de Minsky et de son livre \textit{Society of Mind}, l'architecture n'est pas sans évoqué l'image idéalisé du fonctionnement d'un cerveau tel qu'on se la représente à l'époque en IA symbolique, les CM's ciblant d'ailleurs résolument ce type d'application avec l'implémentation machine d'une version spécialisé de lisp, le *lisp (star lisp). Ainsi même si ce type d’architecture SIMD (dominante sur le marché des HPC jusqu’au années 1990) est généralement plus complexe à appréhender, la CM a par exemple été utilisé très tot pour développer des applications rattachés aux systèmes complexes dans le cadre de l’Artificial Life et des CAS, car elle s’appuie dans sa construction sur la propriété de parallélisme propres aux automates cellulaires (1 processeur = 1 cellule/patch) \Anote{CA_physical}. Les CM proposent certe une alternative intéressante par leur possibilité de parallélisme, mais elle brille aussi par leur performance, la CM-1 (prototype) de 1985-1986 , la CM-2 (commercialisation) de 1986-1987 (2.5 à 6 GFlops), la CM-5 (SIMD vers MIMD, et 1024 processeurs) est numéro 1 du premier \href{http://www.top500.org/featured/systems/cm-5-los-alamos-national-lab/}{@Top500} de 1993 avec 59,7 GFlops sur le Linpack test. En comparaison, à l'IDRIS en 93, le Cray C98 Axis a 8 processeurs dépasse à peine les 7 GFlops.
