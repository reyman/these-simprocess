...
De ce constat, on extrait quelques questions nous permettant à défaut de pouvoir répondre entièrement à cette affirmation, d'y voir au moins un peu plus clair :
...
\item Et, si ce n'est pas le cas, trouve t on une trace de ces pratiques et des challenges associés ? 
...
Cette réflexion commune, mais essentiellement porté par Openshaw, dépasse de beaucoup la seule activité de modélisation, et synthétise aussi les gains génériques potentiels pour qui adopte ce paradigme :
...
En mettant en avant dans nos travaux l'utilité du HPC pour la construction et l'évaluation de modèle de simulation en géographie, il est évident que cette démarche s'inscrit complétement dans ce paradigme.

Certains des gains développés par Openshaw dans son argumentation nécessite un investissement, avec le développement, ou le redéveloppement de certains algorithmes; mais ce qui est plus intéressant ce sont les gain immédiats permis par a) l'utilisation d'algorithmes récent pour résoudre des problématiques autrefois insolubles, b) la réutilisation des algorithmes originaux libéré des contraintes techniques d'autrefois (mémoire, processeur)

Plus concrétement cette pratique s'observe déjà à moindre échelle dans la communauté des modélisateurs agents en SHS, c'est le cas par exemple lorsque \autocite{Brearcliffe2014} tente de reproduire (\textit{reproductibility} et non \textit{replicability} \Anote{reproduire_repliquer}) un des premiers modèle individu-centré développés en 1983 par Hogeweg et Hesper. Il est évident qu'une première exploration avec les moyens moderne d'un modèle autrefois développé sur un mainframe doté d'un processeur à 25Mhz et 4 Mb ne peux qu'apporter de nouveaux éclairage, ne serait ce que parcequ'il est aisé de faire beaucoup plus d'execution du modèle sur la même durée, sans compter tout les bénéfices apportés par un tel changement de plateforme.

En faisant le pari audacieux d'un saut technologique supplémentaire sur ce même modèle, comme nous le conseillerai probablement Openshaw, alors c'est la possibilité d'employer de tout nouveaux algorithmes qui s'offre à nous. La méthode récente de \textit{Pattern Search Exploration} (PSE) \autocite{Cherel2015}, utilisable uniquement dans un environnement HPC moderne, pourrait nous donner une meilleur compréhension de la palette de dynamiques exprimables par le modèle, et cela sans un quelconque re-développement du modèle, et sans définition d'objectifs susceptible de contraindre l'espace de recherche des comportements.

Appliqué à l'activité de modélisation, la simplification (point b)  intervient tout autant dans la démarche KIDS que KISS, à différent niveau, dans le retrait de mécanismes inutile, ou le choix du gradient d'abstraction considéré comme judicieux vis à vis de la problématique donné. En soit, ce n'est pas forcément un problème, au contraire, elle peut être bénéfique si elle est faite de façon intelligente, par exemple lorsqu'il s'agit de remplacer l'expression complexe d'une dynamique micro par une dynamique aggregé macro équivalente. Appliqué à l'exploration des modèles, cette simplification est une contrainte dont on se passerait volontier, car elle met en tension l'interpretation des résultats vis à vis d'une dynamique d'expression incomplète, ou incertaine.


Cette remarque est également valable pour des modèles de simulation plus récents, comme le modèle Luti MobiSim développé par l'équipe Théma. Déjà famillière du mésocentre de calcul de Franche-Comté, il est pourtant indiqué dans la thèse de \textcite{Hirtzel2015} soutenue en février 2015 une limite dans l'utilisation possible des infrastructures disponibles, les simulations s'executant seulement sur des machines de 12 coeurs (5h30 par simulation) ou 16 coeurs (6h30 par simulation). 8 machines ($ x 12$ ou $ x 16$ donc) permettait de faire des simulations en parallèles pour un total de 25 jours de calcul, 790 simulations executés équivalent à 60000 heures de calculs cumulés pour l'analyse de sensibilité présenté. Hirtzel évoque dans sa thèse l'impact important qu'une telle contrainte a eu dans la préparation, la plannification, et la simplification de cette analyse de sensibilité assez complexe devant évaluer l'interaction entre 79 paramètres. L'effort est déjà impressionant, mais pourquoi se contenter de cette infrastructure et ne pas directement \enquote{taper à la porte} des physiciens ou des biologistes disposant d'équipements nationalisés coutant plusieurs dizaine de millions d'euros chacun ? La question semble évidemment naive tant la présence des sciences humaines dans les projets d'avenir concernant le calcul intensif a été évacué ... qui met en lumière un paradoxe, au moment ou l'europe dépense des milliards pour financer un projet de simulateur mondial et social à l'échelle micro, on se demande par quel miracle les sciences humaines vont réapparaitre dans les rapports d'activités francais. (bof) 



% Note sur les modeles simulation plus récents comme le LUTI mobisim.

Il y'aurait encore beaucoup de choses à dire sur la prose tout à fait stimulante d'Openshaw, mais il me semble qu'une seule phrase suffit pour imager tout ces espoirs que nous avons placé dans ces nouveaux usages du HPC pour la simulation.
...
L'idée formidable que la computation, dans la traversée des époque et des modèles, apporte au simple modélisateur de son temps, les moyens de devenir un explorateur toujours plus aguerri dans l'exploration de cet incertain intervenant dans la construction, et l'évaluation des modèles de simulation que nous réalisons.



% Seconde question 

Pour répondre à la seconde question dans un contexte plus francais, cet état de fait n'a pas toujours été vrai, et on a bien vu dans le chapitre 1 que les géographes, et certains modélisateurs pionniers dans les sciences humaines, se sont déjà tournés vers ce qui était alors les rares et uniques ressources informatiques permettant d'être en adéquation avec les enjeux scientifiques de son temps. 
...
Il faut également s'efforcer d'imaginer pendant quelques secondes quelle rupture a pu être l'arrivée physique de la micro-informatique dans certaines pratiques bien installées des sciences humaines. Tout d'un coup un objet un plus gros qu'une calculette apparait sur votre bureau, et permet de réaliser des calculs similaires à ce que vous faisiez en à peine quelques heures de plus sur des superordinateurs.Comme le dit pourtant l'écrivain Cyberpunk William Gibson\Anote{gibson}, la pratique de l'anticipation reste un exercice en définitive beaucoup plus facile que d'imaginer le fonctionnement de sociétés vidées de ces technologies si largement acceptées et répandues aujourd'hui. 
...
Si le HPC est un acronyme à la définition relative, il doit être étudié \textit{in situ} pour mettre face à face challenges scientifiques et moyens computationels au moment ou ils sont disponible. Or il se trouve que dans certains cas, les challenges d'hier sont aussi ceux d'aujourd'hui, et la nécessité d'accès à une ressource informatique de puissance supérieure à un micro, reste comme dans le cas d'exploration des modèles de simulation, toujours une réalité.
...
Enfin, passé cette période ou les pionniers géographes avait pour certain suivi des formations pour la programmation, le paysage universitaire ne semble plus offrir les formations nécessaires.
...
Cette motivation, il me semble que les géographes l'ont trouvé déjà une première fois en françe dans les années 1970, dans l'application notamment de toute nouvelles techniques pour la statistiques ou la simulation, les deux marchant de paire avec la découverte de l'informatique.  Le \textit{supercomputers} a continuer à exister, certains géographes ont continué à l'utiliser, d'autres non, et se sont reposé sur la seule évolution du matériel micro-informatique. 
