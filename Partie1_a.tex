...
\paragraph{Deuxième synergie, avant 2010}

A la différence des projets de modèles de simulation déjà réalisé au laboratoire, le développement de ceux-ci ne seront pas portée par un informaticien. 

On peux identifier trois grand mode de fonctionnement pour la création de modèle de simulation dans un laboratoire :

- Dans un premier cas, il s'agit du mode de construction le plus classique mis en oeuvre par les informaticiens. Les géographes prennent commande d'un modèle, fournissent les informations nécessaires, et des réunions d'avancements sont faites pour ajuster régulièrement. Les acteurs restent dans leur role respectif, l'information est ségmenté et il n'y a pas de transfert de compétence, \autocite[10]{Louail2010} parle d'une collaboration \enquote{faiblement couplé}

- Dans un second cas, il s'agit toujours de réaliser des modèles en géographie par des doctorant en informatique. A cette différence que cette fois-ci la thèse se fait en \enquote{immersion} chez les géographes. Le transfert de compétence n'est pas complet, il s'agit toujours d'une collaboration, mais celle-ci se fait de façon plus étroite, avec la mise en place d'une acculturation progressive entre les deux parties \enquote[11]{Louail2010}. Le travail de Benoit Glisse sur \textbf{Simpop2} et de Thomas Louail sur \textbf{Simpop2}, \textbf{Eurosim} et \textbf{SimpopNano}, tient de ce type de coopération.

- Dans un troisième cas, les géographes s'approprie directement l'outil de modélisation, et développe des compétences en informatique qui leur permettent de nouer un dialogue constructif avec les informaticiens. L'informaticien est toujours utile, mais à terrain de jeu égal, il met sa compétence informatique non pas \enquote{en retrait}, mais \enquote{au service des} problématiques abordés dans le modèle. Le dialogue se construit, se perfectionne, autour d'un objet commun, mais avec des problématiques qui elles ne sont pas nécessairement identique. Arnaud Banos parle d'un processus de co-construction, gagnant-gagnant pour les différentes parties qui accepte les règles du jeu de cette mise en danger disciplinaire.

C'est sur ce troisième mode qu'un groupe de travail se réunit de façon régulière dès janvier 2010, autour de la feuille de route des modèles prévu dans le cadre de la thèse de Clara Schmitt.  Clara Schmitt doit en effet construire plusieurs modèles de simulation (\textbf{SimpopLocal}, \textbf{SimpopNet}, \textbf{SimpopClim}) dans le cadre de son projet de thèse. Composé principalement d'Arnaud Banos, Denise Pumain, Clara Schmitt et moi-même, les réunions de travail s'organisent très vite autour d'un premièr prototype de \textbf{SimpopLocal} en Netlogo. A ce moment là, je pense que tout le monde est convaincu de l'intérét de Netlogo comme support pour cultiver nos idées : Clara est en train de se former rapidement à ce langage, Arnaud est notre maitre à penser par son expérience dans le logiciel, et moi-même je suis plutot partant par une telle équipée. Ayant eu à modifier avec l'aide de Thomas des parti cryptique et non documenté du modèle Simpop 2, développé en Objective-C par un informaticien avant tout accro à l'optimisation;  Netlogo me parait à ce moment là le meilleur vecteur de communication avec les géographes. D'autant plus que c'est sur cette plateforme que j'ai appris à connaitre la simulation agent, et à découvrir au passage ce que pouvait être l'inter-disciplinarité, lors d'un stage encadré au tout début 2009 par Florent Le Néchet, Hélène Mathian, Mathieu Delage, Thomas Louail sur le projet \textbf{AccesSim} \autocite{Delage2010}.

 Les versions vont se succèder sur Netlogo pendant plus de deux ans, en alternant très rapidement entre phase de conception, phase de réalisation, et première phase d'exploration.  

Arnaud Banos est à ce moment là résident à l'institut des systèmes complexe, il encadre un projet CIFRE de simulation avec le projet SIMTRAP de Jéremy Fiegel. En partenariat avec la RATP, l'utilisation de Netlogo est alors un pari pour l'inter-disciplinarité \autocite[53]{Banos2013}


 Simpop est une production commune du groupe de travail, et il ne s'agit ni d'une commande, ni d'un travail encadré par une seule thèse par un informaticien, que celui-ci soit extérieur (Simpop) ou plus en immersion avec les géographes dans le laboratoire (Simpop2 \autocite{}, SimpopNano \autocite{Louail2010}).

Il est prévu initialement que je co-développe le modèle SimpopLocal avec Arnaud Banos et Clara Schmitt. Ce modèle devient le prototype 
